    ...
    <x-card shadow>
        <x-table :headers="$headers" :rows="$projects" :sort-by="$sortBy" with-pagination>
            ...
            <div class="flex gap-2">
                <x-button icon="o-eye" link="{{ route('moderator.projects.show', $project->getRouteKey()) }}" class="btn-ghost btn-sm text-primary" />
{{--                <x-button icon="o-pencil" link="{{ route('moderator.projects.edit', $project->getRouteKey()) }}" class="btn-ghost btn-sm text-primary" />--}}
            </div>
            @endscope
        </x-table>
    </x-card>
